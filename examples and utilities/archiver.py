# Nuke Archiver - Archives the scripts and all the footage in a folder selected by the user
import nuke
import os
import shutil
...
def _archiver(localPath):
	n = nuke.allNodes()
	seqPads = ['%01d', '%02d', '%03d', '%04d', '%05d', '%06d', '%07d', '%08d', '%d', '%1d', '%2d', '%3d']
...
def archiveThisComp():
	...
	if panelResult[0] == 1 and panelResult[1] != '...':
		...
		if _archiver(targetPath):
			nuke.message("Archive complete")
			return True
		else:
			nuke.message("Nothing to archive... canceling")
			return False
